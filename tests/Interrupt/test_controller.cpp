...
class ControllerTest : public ::testing::Test
{
   protected:
    void SetUp() override
    {
        ...
        // Offset (0x1000) + Registers (Banks * sizeof(controlRegisters)) +
        // Mailboxes (Banks * 32 * sizeof(uint64_t)) Let's allocate enough for 2
        // banks to be safe for multi-bank tests. 2 banks registers: 2 * 32
        // bytes = 64 bytes 2 banks mailboxes: 2 * 32 * 8 bytes = 512 bytes
    ...
    void TearDown() override { memory.clear(); }
...
TEST_F(ControllerTest, EnableInterrupt)
{
    ...
    controlRegisters* regs =
        reinterpret_cast<controlRegisters*>(baseAddress + OFFSET);
...
TEST_F(ControllerTest, MailboxOperations)
{
    ...
    uint64_t* mailboxPtr =
        reinterpret_cast<uint64_t*>(mailboxStart + 2 * sizeof(uint64_t));
...
TEST_F(ControllerTest, MultiBankSupport)
{
    ...
    controlRegisters* regs0 =
        reinterpret_cast<controlRegisters*>(baseAddress + OFFSET);
    EXPECT_EQ(regs0->enable_set, (1U << 31));

    // Test Bank 1 (Interrupt 32 -> Bank 1, Bit 0)
    controller.enable(32);
    controlRegisters* regs1 = reinterpret_cast<controlRegisters*>(
        baseAddress + OFFSET + sizeof(controlRegisters));
...
TEST_F(ControllerTest, MultiBankMailbox)
{
    ...
    // mailboxes[i] = reinterpret_cast<uint64_t*>(mailboxBase + i *
    // sizeof(uint64_t)); And getMailbox(interruptNumber) returns
    // *mailboxes[interruptNumber] The loop in constructor I fixed: for
    // (uint32_t i = 0; i < totalBanks * 32; i++) ... So interrupt 35 should
    // simply be index 35.
    ...
    uint64_t* mailboxPtr =
        reinterpret_cast<uint64_t*>(mailboxBase + 35 * sizeof(uint64_t));
